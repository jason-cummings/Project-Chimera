...
int handleFlag(char *arg1, char *arg2, Engine &engine) {
	if (arg1[0] != '-') {
	...
	if (strcmp(arg1, "-l") == 0) {
		...
		if (strcmp(arg2, "") == 0 || arg2[0] == '-') {
			...
			std::cerr << "Valid arguments are: Towers, Castle" << std::endl;
			return 1;
		}
		else {
...
void handleFlags(int argc, char *argv[], Engine &engine) {
	...
	while (i < argc) {
		char *arg1 = argv[i];
		char *arg2 = (i == argc-1) ? "" : argv[i+1];
		i += handleFlag(arg1, arg2, engine);
	}
}

void printHelp() {
	std::cout << "Usage: ./projectchimera [-l <level-name>]" << std::endl << std::endl;
